...
import { changeDuration } from "../../../../actions/actionOrder";
import { format } from "date-fns";

const DateInterval = (props) => {
  const { order, setButtonDisabled } = props;
  ...
  const getInterval = (dateFrom, dateTo) => {
    ...
    if (timeDiff < 0) {
      ...
      const minuts =  Math.abs(dateToFormat.getMinutes() - dateFromFormat.getMinutes());
      return {
